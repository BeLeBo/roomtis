...
def lehrer_page():
import os
path = os.path.join(os.path.dirname(os.path.abspath(**file**)), “lehrer.html”)
...
return jsonify({“ok”: False, “error”: “Falsches Admin-Passwort”}), 403
entry = {
“id”: data.get(“id”) or str(int(**import**(‘time’).time() * 1000)),
“date”: data[“date”],
“title”: data[“title”],
“desc”: data.get(“desc”, “”),
...
return jsonify({“ok”: False, “error”: “invalid uid”}), 400
entries = *load_calendar(f”user*{uid}.json”)
return jsonify({“ok”: True, “entries”: entries})
# POST
data = request.get_json()
uid = data.get(“uid”, “”)
...
entries.append(entry)
*save_calendar(f”user*{uid}.json”, entries)
return jsonify({“ok”: True, “entry”: entry})
